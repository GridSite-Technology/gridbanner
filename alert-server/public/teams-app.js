...
async function loadUsers() {
    ...
    try {
        ...
        container.innerHTML = users.map(user => `
            <div class="user-item" style="border: 1px solid #ddd; padding: 15px; margin-bottom: 10px; border-radius: 4px; background: white;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <div style="font-weight: bold; font-size: 1.1em;">${user.display_name || user.username}</div>
                        <div style="color: #666; font-size: 0.9em;">${user.username}</div>
                    </div>
                    <div style="display: flex; gap: 10px; align-items: center;">
                        <span style="background: #00ADEE; color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.85em;">
...
async function showUserKeys(encodedUsername) {
    ...
    try {
        const keys = await apiCall(`/api/users/${encodedUsername}/keys`);
            ...
            : keys.map(key => `
                <div style="border: 1px solid #eee; padding: 10px; margin-bottom: 10px; border-radius: 4px; background: #f9f9f9;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        ...
                        <button class="btn btn-danger" onclick="deleteUserKey('${encodedUsername}', '${key.id}')" style="padding: 4px 8px; font-size: 0.8em;">Delete</button>
                    </div>
...
async function deleteUserKey(encodedUsername, keyId) {
    showConfirmModal('Delete Key', 'Are you sure you want to delete this key?', async () => {
        try {
            await apiCall(`/api/users/${encodedUsername}/keys/${keyId}`, 'DELETE');
...
async function deleteUser(encodedUsername) {
    const username = decodeURIComponent(encodedUsername);
    showConfirmModal('Delete User', `Are you sure you want to delete user "${username}" and all their keys?`, async () => {
        try {
            await apiCall(`/api/users/${encodedUsername}`, 'DELETE');
